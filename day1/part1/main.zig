...
const INT_SIZE = if (std.mem.eql(u8, INPUT_FILE, "input.txt")) i18 else i5;
...
pub fn main() !void {
    ...
    std.mem.sort(INT_SIZE, &left_nums, {}, std.sort.asc(INT_SIZE));
    std.mem.sort(INT_SIZE, &right_nums, {}, std.sort.asc(INT_SIZE));

    var sum: u64 = 0;
    for (left_nums, 0..) |_, i| {
        sum += @abs(left_nums[i] - right_nums[i]);
        std.debug.print("left: {d}\nright: {d}\nrunning sum: {d}\n", .{ left_nums[i], right_nums[i], @abs(left_nums[i] - right_nums[i]) });
    }
    std.debug.print("\nfinal sum: {d}\n", .{sum});
}
